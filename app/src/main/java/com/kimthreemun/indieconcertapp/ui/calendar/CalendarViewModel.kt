...
class CalendarViewModel : ViewModel() {
    ...
    private val _filteredPerformances = MutableLiveData<List<Performance>>()
    val filteredPerformances: LiveData<List<Performance>> = _filteredPerformances

    val allPerformances = listOf(
        Performance(
            id = 1,
    ...
    fun selectDate(date: LocalDate) {
        ...
        _filteredPerformances.value = allPerformances.filter {
    ...
    fun getPerformanceDates(): Set<LocalDate> {
        // 지역 필터 고려
        val regions = _selectedRegions.value.orEmpty()
